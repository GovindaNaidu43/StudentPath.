        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            <h2 className="text-4xl font-black tracking-tight text-white mb-4">
              StudentPath<span className="text-fuchsia-400">âœ¦</span>
            </h2>
          ...
          <div>
            ...
            <ul className="space-y-3.5">
              {[{ label: "About Us", href: "/about" }, { label: "Careers", href: "/explore" }, { label: "Contact Us", href: "#" }, { label: "Privacy Policy", href: "#" }].map((item) => (
                <li key={item.label}>
                  <Link href={item.href} className="text-zinc-400 hover:text-fuchsia-300 transition-all duration-300 text-sm font-medium">{item.label}</Link>
                </li>
              ))}
            </ul>
          </div>
    ...
    {[
      ...
      <li key={item.name}>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-medium text-zinc-400">
          <p>Â© 2026 StudentPath Inc. All rights reserved.</p>
          <p className="flex items-center gap-1.5">
            Designed with <span className="text-fuchsia-400">â™¥</span> for the next generation of students.
          </p>
        </div>
      </div>
    </footer>
